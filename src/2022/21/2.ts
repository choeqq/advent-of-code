...
const getValue = (of: Monkey, monkeys: MonkeyMap): number => {
	if (of.operation === undefined) return of.value;
...
const getSideWithHuman = (
	...
	if (monkey.operation === undefined) return 'nowhere';
	...
	if (leftAnswer === 'nowhere' && rightAnswer !== 'nowhere') return 'right';
	if (rightAnswer === 'nowhere' && leftAnswer !== 'nowhere') return 'left';
	return 'nowhere';
};
