...
const endPos = [
	input.length - 1,
	input[input.length - 1].findIndex((l) => l === '.'),
];
...
const blizzards: Blizzard[] = input.flatMap((l, i) =>
	l
		.map((c, j) => {
			switch (c) {
				case '^':
...
const howManyMinutesToFinish = (
	...
	while (toCheck.size > 0) {
		...
		for (const pos of toCheck) {
			...
			if (!nextOccupied.has(pos)) nextToCheck.add(pos);

			const currNumPos = pos.split(',').map(Number);
			directions.forEach((direction) => {
				const nextPos = [
					currNumPos[0] + direction[0],
					currNumPos[1] + direction[1],
				].toString();
...
const timeBackToStart = howManyMinutesToFinish(
	endPos,
	startPos,
	firstTimeToCross + 1
);
const timeBackToFinish = howManyMinutesToFinish(
	startPos,
	endPos,
	timeBackToStart + 1
);
