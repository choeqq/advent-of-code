...
const getNewDirection = (oldDir: number, instruction: string): number => {
	if (instruction === 'R') {
		if (oldDir === 3) return 0;
		return oldDir + 1;
	} else {
		if (oldDir === 0) return 3;
		return oldDir - 1;
	}
};
...
const movement = input[input.length - 1].match(/\d+|[A-Z]/g)?.map((instr) => {
	if (instr.match(/\d+/)) return Number(instr);
	return instr;
}) as Instruction[];

const height = input.length - 1;

const inputMap = input.slice(0, height).map((l) => {
...
const answer =
	1000 * (finalPlayer.loc.pos[0] + 1 + currOffset[0]) +
	4 * (finalPlayer.loc.pos[1] + 1 + currOffset[1]) +
