...
const northCheck = (elf: Elf): number[] | undefined => {
	if (
		elf.neighbours[0] === undefined &&
		elf.neighbours[1] === undefined &&
		elf.neighbours[2] === undefined
	) {
		// North
...
const southCheck = (elf: Elf): number[] | undefined => {
	if (
		elf.neighbours[4] === undefined &&
		elf.neighbours[5] === undefined &&
		elf.neighbours[6] === undefined
	) {
		// South
		return [elf.pos[0] + 1, elf.pos[1]];
	}
...
const finalElves = Array.from({ length: howManyRounds }, (_, i) => i).reduce(
	(elves, round) => {
