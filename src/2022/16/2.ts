...
const sortPathByPressure = (a: Path, b: Path) =>
	b.finalPressure - a.finalPressure;
...
const getMostPressureReleaseWithElephant = (
	...
	for (let humanPath = 0; humanPath < paths.length; humanPath++)
		for (
			let elephantPath = humanPath + 1;
			elephantPath < paths.length;
			elephantPath++
		)
			if (
	...
	for (let n = 0; n < paths.length; n++) {
		...
		path.toVisit.forEach((room) => {
			if (room !== path.curr && path.timeLeft - currPrices[room] > 1) {
				madeNewPath = true;
				const newTimeAfterVisitAndValveOpen =
					path.timeLeft - currPrices[room] - 1;
				paths.push({
					curr: room,
					...
					finalPressure:
						path.finalPressure +
						newTimeAfterVisitAndValveOpen * roomsFromName[room].flow,
				});
				paths.push({
					curr: room,
					toVisit: [],
	...
	while ((curr = toVisit.shift())) {
		...
		worthItAdj.forEach((neighbour) => {
			...
			const costToNeighbour =
				lowestCost[neighbour.name] === undefined
					? newCostToNeighbour
					: lowestCost[neighbour.name];
...
const inputRooms: InputRoomMap = input.reduce(
	(map, currRoom) => ({
		...map,
...
const roomsMovePrices: PricesRoomMap = startingRooms.reduce(
	(map: PricesRoomMap, room: Room) => {
		return {
			...map,
			[room.name]: dijkstra(
				room,
