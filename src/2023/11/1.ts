...
interface Monkey {
	...
	toMonkeyIfTestTrue: number;
	toMonkeyIfTestFalse: number;
}
...
for (let i = 0; i < NUM_MONKEYS; i++) {
	...
	for (let j = 0; j < NUM_LINES_PER_MONKEY; j++) {
		const line = inputArray[i * NUM_LINES_PER_MONKEY + j];
		if (j === 0) {
			id = parseInt(line[7], 10);
		} else if (j === 1) {
			const numbersStr = line.substring(line.indexOf(': ') + 2);
...
const monkeyMap = new Map<number, number>();
/*
	...
	for (const monkey of monkeys) {
		...
		for (let itemUnderInspection of items) {
			...
			if (itemUnderInspection % monkey.testDivisible === 0) {
				monkeys[monkey.toMonkeyIfTestTrue].startingItems.push(
					itemUnderInspection,
				);
			} else {
				monkeys[monkey.toMonkeyIfTestFalse].startingItems.push(
					itemUnderInspection,
				);
			}
		}
